drawHeatmap <- function(mirna_data, cutoff, surv = FALSE){
  ...
  if(surv){
    ...
    long_term_survival["CIMP"] <- 
      clinical_data[match(long_term_survival$Sample,
                          clinical_data$Sample), "CIMP_status"]
    long_term_survival["IDH1"] <- 
      clinical_data[match(long_term_survival$Sample,
                          clinical_data$Sample), "IDH1_status"]
    long_term_survival["MGMT"] <- 
      clinical_data[match(long_term_survival$Sample,
                          clinical_data$Sample), "MGMT_status"]
    long_term_survival$Survived_12_Months <- rep("Yes", 
                                                 nrow(long_term_survival))
    ...
    short_term_survival["CIMP"] <- 
      clinical_data[match(short_term_survival$Sample,
                          clinical_data$Sample), "CIMP_status"]
    short_term_survival["IDH1"] <- 
      clinical_data[match(short_term_survival$Sample,
                          clinical_data$Sample), "IDH1_status"]
    short_term_survival["MGMT"] <- 
      clinical_data[match(short_term_survival$Sample,
                          clinical_data$Sample), "MGMT_status"]
    short_term_survival$Survived_12_Months <- rep("No", 
                                                  nrow(short_term_survival))
    ...
    nrow(mirna_data_with_survival[mirna_data_with_survival$CIMP == "G-CIMP", ])
    ...
    # Sukuriamas heatmap anotacijų objektas.
    ...
    # Heatmap vaizduojamas kartu su prieš tai sukurta anotacija.
    max_text_width(1)
